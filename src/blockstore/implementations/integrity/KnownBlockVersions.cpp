...
KnownBlockVersions::KnownBlockVersions(const bf::path &stateFilePath, uint32_t myClientId)
        :_integrityViolationInLastRun(false), _knownVersions(), _lastUpdateClientId(), _stateFilePath(stateFilePath), _myClientId(myClientId), _mutex(), _valid(true) {
...
KnownBlockVersions::KnownBlockVersions(KnownBlockVersions &&rhs) // NOLINT (intentionally not noexcept)
        : _integrityViolationInLastRun(false), _knownVersions(), _lastUpdateClientId(), _stateFilePath(), _myClientId(0), _mutex(), _valid(true) {
    unique_lock<mutex> rhsLock(rhs._mutex);
    unique_lock<mutex> lock(_mutex);
    _integrityViolationInLastRun = rhs._integrityViolationInLastRun;
...
bool KnownBlockVersions::integrityViolationInLastRun() const {
    return _integrityViolationInLastRun;
}
...
void KnownBlockVersions::_loadStateFile() {
    ...
    _integrityViolationInLastRun = deserializer.readBool();
    _knownVersions = _deserializeKnownVersions(&deserializer);
    _lastUpdateClientId = _deserializeLastUpdateClientIds(&deserializer);
...
void KnownBlockVersions::_saveStateFile() const {
    ...
    serializer.writeBool(_integrityViolationInLastRun);
